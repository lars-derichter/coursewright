 ...
 *   theme: string, export: { style: string, number_headings: boolean },
 ...
 *   names or paths, resolved to files by their consumers.
 *   `export.number_headings` is whether PDF and DOCX exports number their
 *   headings. `checks` is what
...
function loadCourseConfig(rootDir) {
  ...
  for (const key of Object.keys(exportSettings)) {
    if (key !== 'style' && key !== 'number_headings') {
      log.warn(
  ...
  const config = Object.freeze({
    ...
    export: Object.freeze({
      ...
      number_headings: readFlag(
        exportSettings.number_headings,
        false,
        'export.number_headings',
      ),
    }),
...
/**
 * Read an optional boolean. YAML already turns `true`/`false` into booleans, so
 * anything else is a typo (a quoted "false", a stray word) and warns rather
 * than being coerced: "false" would otherwise switch the feature on.
 */
function readFlag(value, fallback, keyName) {
  if (value == null) return fallback;
  if (typeof value !== 'boolean') {
    log.warn(
      `[course-config] Ignoring "${keyName}" in ${CONFIG_FILENAME}: expected true or false`,
    );
    return fallback;
  }
  return value;
